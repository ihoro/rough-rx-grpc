...
module.exports = class RxGrpc {
  ...
  closeAllClients() {
    Object.getOwnPropertyNames(this.serviceClients).forEach(name => {
      this.serviceClients[name].grpcClient.close();
      delete this.serviceClients[name];
    });
  }
  ...
  withProtoFiles(protoFileGlobs) {
    ...
    if (Array.isArray(protoFileGlobs))
      toAdd = protoFileGlobs;
    else
      toAdd = [ protoFileGlobs ];

    toAdd.forEach(protoFileGlob => {
      if (! this.protoFileGlobs_loaded.includes(protoFileGlob))
        this.protoFileGlobs.push(protoFileGlob);
    });
  ...
  _loadProtos() {
    ...
    return this._scanProtoFileGlobs(this.protoFileGlobs).pipe(
      flatMap(protoFile => {
        const cachedProto = cachedProtos[protoFile];
        if (cachedProto) {
          this.protos.push(cachedProto);
          return of(cachedProto);
        }
        return this._loadProto(protoFile).pipe(
          tap(_ => this.protoFileGlobs_loaded.push(protoFile))
        );
      }),
      count(),
  ...
  _loadProto(protoFilePath) {
    return from(protoLoader.load(protoFilePath, this.options.protoLoader)).pipe(
      ...
      tap(proto => {
        ...
        cachedProtos[protoFilePath] = proto;
      }),
    );
  }
